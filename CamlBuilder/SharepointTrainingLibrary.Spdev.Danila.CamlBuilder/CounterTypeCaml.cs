...
namespace SharePointTraining.Spdev.Danila.CamlBuilder.CamlTypes
{
    ...
    public class CounterTypeCaml : IntegerTypeCaml
    {
        private readonly string _fieldName;

        private readonly Guid _guid;

        public CounterTypeCaml(string fieldName) : base(fieldName)
        {
            this._fieldName = fieldName;
        }


        public CounterTypeCaml(Guid guidName) : base(guidName)
        {
            this._guid = guidName;
        }
        ...
        public override XElement CreateConditionWithValues(XElement xElement, params object[] values)
        {
            ...
            if (values.Length == 1)
            {
                xElement.Add(this.ValueElement(TypeValue.Counter, values[0].ToString()));
            }
            else
            {
                foreach (int value in values)
                {
                    xElement.Add(this.ValueElement(TypeValue.Counter, value.ToString()));
                }
            }
